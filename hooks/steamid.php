...
abstract class steam_hook_steamid extends _HOOK_CLASS_
{
	...
	public function form( &$form )
	{
		try
		{
			try
			{
				...
				static::$additionalFieldToggles['Steamid'] = array( 'pf_not_null', 'pf_max_input', 'pf_input_format', 'pf_search_type', "{$form->id}_header_pfield_displayoptions" );
				parent::form( $form );
	...
	protected function canKeepValueOnChange( $newType )
	{
		try
		{
			if( $newType == 'Steamid')
			{
				return in_array( $this->type, array( 'Email', 'Password', 'Text', 'Tel', 'Url', 'Editor', 'TextArea', 'Code', 'Codemirror', 'Steamid' ) );
			}
			if( $this->type == 'Steamid')
			{
				return in_array( $newType, array( 'Email', 'Password', 'Text', 'Tel', 'Url', 'Editor', 'TextArea', 'Code', 'Codemirror', 'Steamid' ) );
			}
