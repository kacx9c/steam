...
class _manage extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		$options = array( 	'update' => 'steam_multi_update',
	...
	public function update()
	{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect( \IPS\Http\Url::internal('app=steam&module=steam&controller=manage&do=update'), function( $doneSoFar ) use($perGo)
			{
...
*/
			...
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=steam&module=steam&controller=manage' ), $e->getMessage() );
			}

				return array( $doneSoFar, \IPS\Member::loggedIn()->language()->addToStack('steam_update_running'), ($doneSoFar / $cache['count']) * 100 );
			}, function()
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=steam&module=steam&controller=manage' ), 'steam_update_complete' );
			} );
	...
	public function cleanup()
	{
		...
		\IPS\Output::i()->output = new \IPS\Helpers\MultipleRedirect( \IPS\Http\Url::internal('app=steam&module=steam&controller=manage&do=cleanup'), function( $doneSoFar ) use($perGo)
			{
			try
			{
				$steam = new \IPS\steam\Update;
			...
			}catch(\Exception $e)
			{
				...
				return array( $doneSoFar, \IPS\Member::loggedIn()->language()->addToStack('steam_cleanup_running'), ($doneSoFar / $cache['count']) * 100 );
			}, function()
			{
				\IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=steam&module=steam&controller=manage' ), 'steam_cleanup_complete' );
			} );
