...
// https://partner.steam-api.com/ISteamUser/GrantPackage/v1/?key=C32DD550FE59CED2A8C6A288796243DC&steamid=76561197964468370
	...
	public function updateProfile($single = 0)
	{
		try
		{
			...
			for( $i = 0; $i < $profile_count; $i++)
			{
				...
				$query = \IPS\Db::i()->select( $select, array('steam_profiles', 's'), $where, 's.st_member_id ASC', array( $this->extras['profile_offset'], 100), NULL, NULL, '011');

				foreach($query as $id => $row)
				{
					$ids[$id] = $row['st_steamid'];
					$profiles[$id] = array( 	'member_id' => $row['st_member_id'],
	...
	*/
	...
	public function getFieldID()
	{
		try
		{
			$this->cfID = \IPS\DB::i()->select( 'pf_id,pf_group_id' , 'core_pfields_data', array( 'pf_type=?', 'Steamid'))->first();
		}
		catch( \Exception $e)
		{
